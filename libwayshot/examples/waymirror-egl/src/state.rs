...
impl WaylandEGLState {
    ...
    pub fn init_egl(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.init_program()?;
        let (frame_format, _guard, bo) = self
            .wayshot
        ...
        let image_attribs = [
            ...
            dbg!(bo.fd_for_plane(0).unwrap().as_raw_fd()) as Attrib,
            0x3273,
        ...
        unsafe {
            ...
            ) -> khronos_egl::EGLImage = std::mem::transmute(dbg!(self
                .egl
            ...
            dbg!(image, self.egl.get_error().unwrap());
    ...
    fn init_program(&mut self) -> Result<()> {
        ...
        let vertices: [gl::types::GLfloat; 12] = [
