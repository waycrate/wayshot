...
fn main() -> Result<()> {
    ...
    match settings.command {
        ...
        #[cfg(feature = "color_picker")]
        Command::ColorPicker(format) => {
            color_picker::pick(&mut connection, settings.freeze, format)
        }
        Command::Screenshot(mode) => {
            ...
            let result =
                screenshot::capture(&mut connection, &mode, settings.cursor, settings.freeze);
            match result {
